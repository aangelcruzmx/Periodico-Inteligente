...
def _transicion(posicion: int, es_ultima: bool) -> str:
    """
    Devuelve una frase de transición natural según la posición de la noticia
    dentro de su bloque de categoría. Usa un pool amplio con rotación por
    módulo para evitar repeticiones consecutivas.
    """
    if posicion == 0:
        return ""
    ...
    if es_ultima:
        pool_cierre = [
            "Y para cerrar esta sección, ",
            "Como última noticia en este apartado, ",
            "Para finalizar este bloque, ",
            "Cerramos esta sección con: ",
        ]
        return pool_cierre[(posicion - 1) % len(pool_cierre)]
    ...
    pool_continuacion = [
        ...
        "Nos llega también que ",
        "De igual forma, ",
        "Mientras tanto, ",
        "Y atención a esto: ",
    ]
    return pool_continuacion[(posicion - 1) % len(pool_continuacion)]
...
def construir_texto_podcast(noticias_analizadas: list[dict]) -> str:
    ...
    partes.append(
        ...
        "Recuerda que puedes encontrar más detalles y enlaces a los artículos en la app. "
