    ...
    if not hay_datos:
        ...
        if not datos_graf:
            ...
            if conteo_sent and total_noticias > 0:
                porcentaje_dominante = round(
                    (conteo_sent.most_common(1)[0][1] / total_noticias) * 100
                )
            ...
            # ── Lenguaje del día ─────────────────────────────────────────────
            palabras_top, bigramas_top = obtener_palabras_y_bigramas(datos_graf, top_n=10)

            col_lenguaje_1, col_lenguaje_2 = st.columns([1, 1])
            ...
            with col_lenguaje_1:
                ...
                if palabras_top:
                    ...
                    st.bar_chart(df_palabras.set_index("Palabra")["Frecuencia"])
                    st.caption(
                        "Estas palabras aparecen con mayor frecuencia en titulares y resúmenes de la edición actual."
                    )
                else:
            ...
            with col_lenguaje_2:
                st.markdown('<div class="section-card">', unsafe_allow_html=True)
                st.subheader("🔎 Expresiones frecuentes")

                if bigramas_top:
                    df_bigramas = pd.DataFrame(bigramas_top, columns=["Expresión", "Frecuencia"])
                    st.dataframe(
                        df_bigramas,
                        use_container_width=True,
                        hide_index=True
                    ...
                    st.caption(
                        "Estas combinaciones de dos palabras ayudan a detectar temas o enfoques repetidos."
                    )
                else:
                    st.info("No hay suficientes expresiones para analizar.")

                st.markdown('</div>', unsafe_allow_html=True)

            # ── Mapa editorial: categoría × sentimiento ──────────────────────
            st.markdown('<div class="section-card">', unsafe_allow_html=True)
            st.subheader("🧩 Mapa editorial: categoría × sentimiento")

            filas_mapa = []
            for n in datos_graf:
                filas_mapa.append({
                    "Categoría": n.get("categoria", "otros").capitalize(),
                    "Sentimiento": NOMBRES_SENTIMIENTO.get(
                        n.get("sentimiento", "neutral"),
                        "Neutro"
                    )
                })
            ...
            if filas_mapa:
                ...
                tabla_cruzada = pd.crosstab(df_mapa["Categoría"], df_mapa["Sentimiento"])

                st.dataframe(
                    tabla_cruzada.reset_index(),
                    use_container_width=True,
                    hide_index=True
                )

                st.caption(
                    "Este cruce muestra cómo se reparte el tono editorial dentro de cada categoría."
                )
            else:
                st.info("No hay datos suficientes para construir el mapa editorial.")
